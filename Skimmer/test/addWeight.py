...
from boostedWScalefactorProducer.Skimmer.datasets import dictSamples, checkDict
...
LUMI = checkDict('SingleMuon',dictSamples)['2017']['lumi']
...
def processFile(sample_name, verbose=False):
    ...
    fileKey = sample_name.replace("_nanoskim","")
...
#        return True
    ...
    if isMC:
        ...
        # Cross section
        #XS = getXsec(sample)
        XS = checkDict(fileKey,dictSamples)['XS']
        #SF = getSF(sample)
    ...
    for event in range(0, tree.GetEntries()):
        ...
        # Weights
        if isMC:
            eventweightlumi[0] = Leq * tree.eventWeight
